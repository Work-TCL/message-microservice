...
import { CreatorModel, NotificationModel, VendorModel } from "../../database/model";
...
export const notificationList = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        const unreadCount = await NotificationModel.countDocuments({ userId: _id, read: false });
...
export const getUnreadNotifications = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        const unreadNotificationsCount = await NotificationModel.countDocuments({ userId: _id, read: false });
...
export const markRead = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (readAll) {
            ...
            const { _id } = req.user;
            await NotificationModel.updateMany({ userId: _id, read: false }, { read: true });
